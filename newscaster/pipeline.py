...
def main(fake=False):
    ...
    today = date.today()
    super_date_format = format_spoken_date(today)

    print_and_write(super_date_format)
    formatted_date = today.strftime("%B %d, %Y")
    formatted_date2 = today.strftime("%Y_%m_%d")
    formatted_date3 = today.strftime(super_date_format)

    ####INFO GATHER ################################################################
    if fake:
        filename = "segment_summaries/everything_is_fake_lololol.txt".format(formatted_date2)
    else:
...
def super_main():
    ...
    if real_run:
        print_and_write('waiting for 4')
        while run:
            now = datetime.now()
            if now.hour >= 5 and now.hour < 24:
                print_and_write("It's 5:00, time to run the program!")
                main()
                run = False
            else:
                time.sleep(10)
