...
RESEARCH_CONTROLLER_PROMPT = (
    ...
    "Evidence sufficiency is not your only test. Also ask what an attentive listener would "
    "immediately wonder that the evidence does not answer. A duration, cost, deadline, count, "
    "or decision that is well attributed but never explained is a real gap, even though the "
    "claim itself needs no further verification: 'officials say it will take a year' is "
    "sourced but unexplained, and a listener will ask why. One mechanism_check on the single "
    "most glaring unexplained figure is worth more than declaring the story done.\n\n"
    "Order of priority: a genuinely unresolved current-events question at the centre of the "
    "story comes first, so long as you have not already asked it. Only once that is answered, "
    "or has proven unresolvable because the event has not happened yet, should you spend an "
    "iteration explaining an unexplained figure. Do not let an explanatory question crowd out "
    "a live one you have never asked.\n\n"
    "Read RECENT COMPLETED Q&A before you choose. Do not re-ask a question that has already "
    "been asked. If an earlier question came back unresolved twice, asking a third time will "
...
INTRO_PROMPT_TEMPLATE = (
    ...
    "that {weather} Keep every temperature and both place names. Mention rain only if that sentence mentions it. Then, briefly mention the upcoming stories that will be covered in the episode, which are listed below. "
    ...
    "would be a good text to make:\"La Jolla will reach eighty-one degrees today, about seventy-six at midday and seventy-eight "
    "by five, so keep water in the car. Back in Temecula it cools to the low seventies this evening with an overnight low of "
    "sixty-two. It's a perfect day to reflect on the "
